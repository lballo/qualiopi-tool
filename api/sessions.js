...
module.exports = handler(async (req, res) => {
  ...
  if (action === "creer") {
    ...
    /* Les dates de la session ne sont plus stockées : elles remontent des créneaux
       (rollups « Première demi-journée » et « Dernière demi-journée »).
       Seule la durée en jours reste une donnée propre à la session. */
    if (info.jours) {
      await notion(`/pages/${sessionId}`, "PATCH", {
        properties: { "Durée (jours)": W.num(info.jours) },
      });
    }
